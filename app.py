...
def load_default_data():
    ...
    try:
        return pd.read_csv('greenlens_esg_dataset.csv')
    except FileNotFoundError:
        st.error("Default dataset 'greenlens_esg_dataset.csv' not found. Please upload a file to begin.")
        return None
...
with st.sidebar.expander(":material/info: How data loading works"):
    st.markdown("""
    - **By default,** this app displays the built-in 'GreenLens' dataset.
    - **To analyze your own data,** just upload a CSV file using the button above.
    - The report will automatically update to reflect your file's data.
    """)
# --- END NEW ---

st.sidebar.markdown("<hr style='margin: 1em 0;'>", unsafe_allow_html=True) # Visual separator
...
if uploaded_file is not None:
    ...
    st.sidebar.info("Showing default 'GreenLens' dataset. Upload a file to see your own data.")


# --- MAIN DASHBOARD (wrapped in error handling) ---
if df is None:
    st.warning("Data could not be loaded. Please upload a valid CSV file to start the analysis.")
else:
    try:
        # Sidebar filters
        ...
        selected_years = st.sidebar.multiselect(
            "Select Years",
            years,
            default=years[:2] if len(years) > 1 else years
        )
        ...
        selected_departments = st.sidebar.multiselect(
            "Select Departments",
            departments,
            default=departments[:3] if len(departments) > 2 else departments
        )
        ...
        if filtered_df.empty:
            ...
            tab1, tab2, tab3, tab4, tab5 = st.tabs([
                ":material/analytics: Environmental",
                ":material/groups: Social",
                ":material/gavel: Governance",
                ":material/trending_up: Trends",
                ":material/table_view: Detailed View"
            ])
            ...
            with tab1:
                ...
                # Environmental KPIs
                st.markdown("#### :material/public: Key Environmental Intensity Metrics")
                ...
                # Row 1: Energy & Carbon
                st.markdown("#### :material/electric_bolt: Energy & Carbon Performance")
                ...
                st.markdown("#### :material/recycling: Waste & Recycling Performance")
                col1, col2 = st.columns(2)
                ...
                # Row 3: Water Usage
                st.markdown("#### :material/water_drop: Water Usage Performance")
                col1, col2 = st.columns(2)
                ...
                st.markdown("#### :material/park: Combined Environmental Performance")
                col1, col2 = st.columns(2)
